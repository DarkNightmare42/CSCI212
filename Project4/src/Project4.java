...
public class Project4 {
	...
	private static SortedCandleList sorted;
	private static UnsortedCandleList unsorted;
	
	//@SuppressWarnings("unused")
	public static void main(String args[]) {
		newWindow = new CandleGUI(500, 400,"CandleGUI Project4");
	}
	...
	public void searchString(float criteria) {
		if(unsorted == null) { //checks that the list isn't empty, if it is call the readIn method
			JOptionPane.showMessageDialog(null, "No source file selected, \nKindly choose a source file to continue.","File Selection", JOptionPane.WARNING_MESSAGE);
			readIn();
		}
		newWindow.searchOutput(sorted, unsorted, criteria); //sends criteria and list to gui to carry out task
	}//search method
	...
	private static String getPath() { 
		...
		int status = candleFile.showOpenDialog(null);
		if (status == JFileChooser.APPROVE_OPTION) 
			file = candleFile.getSelectedFile(); //retrieves file path once selected
		else {
			JOptionPane.showMessageDialog(null, "Open File dialog canceled \nProgram Will Exit","Canceled",JOptionPane.WARNING_MESSAGE);
